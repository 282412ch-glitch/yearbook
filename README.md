...
当前是 **75% 节点开发版**：记录、照片、时间轴/月历、中文搜索、回顾补记、第一次、盲盒、备份恢复、手动年册及导出任务；新增双协议模型配置、能力验证、AI 整理、月报、Agent、草稿版本及来源回查。真实模型 API 尚未验证，未来信和完整 PDF/导出排版验收留待最终节点；参见 [实施计划](docs/PLAN.md)。本节点结束后按用户要求暂停，续接阅读 [HANDOFF](docs/HANDOFF.md)。
...
- 模型能力以逐项验证结果为准。当前自动化使用本机模拟服务，没有验证真实 Key、供应商兼容性或实际生成质量。
