...
import { BookOpen, CalendarDays, FileText, House, Mail, Menu, Settings2, Shuffle, Sprout, X } from 'lucide-react';
import { HomePage, MemoriesPage, NotFoundPage, RecordDetailPage, RecordsPage, SettingsPage, StagePage } from './pages';
...
export function App() {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">跳到正文</a><header className="mobile-header"><Link to="/" className="brand"><img src="/brand-mark.png" alt="" width={32} height={36} /><span>一年一册</span></Link><button className="icon-button" aria-label={menuOpen ? '收起导航' : '打开导航'} aria-expanded={menuOpen} aria-controls="main-navigation" onClick={() => setMenuOpen(value => !value)}>{menuOpen ? <X size={24} /> : <Menu size={24} />}</button></header>
    <aside className={`sidebar ${menuOpen ? 'open' : ''}`}><Link to="/" className="brand desktop-brand"><img src="/brand-mark.png" alt="" width={32} height={38} /><div><span>一年一册</span><small>日子有迹可循</small></div></Link><nav id="main-navigation" className="main-nav" aria-label="主要导航">{navigation.map(({ to, label, icon: Icon, end }) => <NavLink key={to} to={to} end={end} className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`}><Icon size={20} strokeWidth={1.7} /><span>{label}</span></NavLink>)}</nav><div className="sidebar-bottom"><NavLink to="/settings" className={({ isActive }) => `nav-link ${isActive ? 'active' : ''}`}><Settings2 size={20} strokeWidth={1.7} />设置与备份</NavLink><div className="sidebar-note"><span className="local-indicator" />在这台电脑里，好好保存。</div><Link to="/tasks" className="version-link">任务与生成进度</Link></div></aside>
    <div className="main-shell"><main id="main-content" tabIndex={-1}><Routes><Route path="/" element={<HomePage />} /><Route path="/records" element={<RecordsPage />} /><Route path="/records/new" element={<RecordEditorPage />} /><Route path="/records/:id" element={<RecordDetailPage />} /><Route path="/records/:id/edit" element={<RecordEditorPage />} /><Route path="/firsts" element={<RecordsPage firstOnly />} /><Route path="/memories" element={<MemoriesPage />} /><Route path="/trash" element={<RecordsPage deleted />} /><Route path="/settings" element={<SettingsPage />} /><Route path="/yearbooks" element={<YearbookListPage />} /><Route path="/yearbooks/new" element={<YearbookEditorPage />} /><Route path="/yearbooks/:id/edit" element={<YearbookEditorPage />} /><Route path="/yearbooks/:id/preview" element={<YearbookPreviewPage />} /><Route path="/reports" element={<ReportsPage />} /><Route path="/ai" element={<AiComposePage />} /><Route path="/ai/drafts/:id" element={<AiDraftPage />} /><Route path="/letters" element={<StagePage kind="letters" />} /><Route path="/tasks" element={<TasksPage />} /><Route path="*" element={<NotFoundPage />} /></Routes></main><footer className="app-footer"><span>一年一册</span><span>记下平常，留给以后。</span><Link to="/settings">本机保存与备份</Link></footer></div>
  </div>;
}
